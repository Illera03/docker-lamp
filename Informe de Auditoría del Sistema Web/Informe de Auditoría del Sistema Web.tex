...
En el apartado de inyección se han detectado dos amenazas de riesgo alto. Las vulnerabilidades encontradas son Cross-Site Scripting e Inyección SQL, como se muestra en la Figura \ref{fig:xss_error}.
\begin{figure}[h]
	\centering
	\fbox{\includegraphics[width=0.7\textwidth]{figures/injection_error.png}} % Imagen con borde negro
...
En este caso, la vulnerabilidad fue detectada en campos de entrada como la fecha de lanzamiento y otros campos en el formulario de modificación de los juegos.\\
\textbf{Causa de la Vulnerabilidad}

La vulnerabilidad XSS detectada se originaba en el campo de la fecha de lanzamiento de los juegos y otros campos de entrada en el formulario de modificación. Esto se debía a que los valores de entrada eran reflejados en la respuesta sin la sanitización o validación adecuada, permitiendo a los atacantes inyectar y ejecutar código JavaScript en el navegador del usuario.
...
Después de implementar soluciones: Tras aplicar la sanitización y validación, el escaneo de ZAP no encontró vulnerabilidades de XSS, lo que confirmó que el problema estaba resuelto.
...
\begin{itemize}
	\item 
	\textbf{Sanitización de Entrada y Escapado de Salida:\\}
	Se utilizó la función \code{htmlspecialchars()} en PHP para escapar caracteres especiales (como <, >, `I\&', ", ') en todas las salidas. Esto asegura que el contenido reflejado en el navegador sea tratado como texto y no como código ejecutable.
	...
	\item \textbf{Sanitización de Datos Entrantes con \code{mysqli\_real\_escape\_string}:\\} Para cada valor de entrada recibido a través del formulario, se aplicó \code{mysqli\_real\_escape\_string()} para evitar que cualquier código especial o malicioso llegue a la base de datos.
...
La vulnerabilidad \textbf{CWE-89: Inyección SQL} también se ha solucionado con estos cambios.


\subsubsection{Inyección SQL}
\textbf{Descripción:} La vulnerabilidad CWE-89 corresponde a la inyección SQL, donde un atacante es capaz de insertar o manipular consultas SQL a través de los datos de entrada, en este caso, el parámetro \texttt{id} en la URL. En el error encontrado, la inyección fue posible al modificar el parámetro de la siguiente manera:

\begin{lstlisting}
	1750' AND '1'='1' --
\end{lstlisting}

Esto permitió al atacante modificar la consulta SQL ejecutada en el servidor, manipulando los resultados devueltos y accediendo a información que no debía ser revelada. En particular, se manipuló la consulta para obtener datos utilizando una condición booleana que siempre era verdadera, \texttt{'1'='1'}, y luego se comentó el resto de la consulta utilizando \texttt{--}, lo que invalidaba el resto de la consulta SQL.

\textbf{Causa de la Vulnerabilidad:} La vulnerabilidad fue causada por la construcción dinámica de consultas SQL sin la adecuada sanitización ni parametrización de los datos proporcionados por el usuario. En este caso, el valor del parámetro \texttt{id} se concatenó directamente a la consulta SQL sin protección, lo que permitió la ejecución de código SQL malicioso.


\subsubsection{Soluciones implementadas}
\begin{itemize}
	\item Utilizar consultas SQL parametrizadas para evitar la inyección de código malicioso.
	\item Implementar la sanitización de todas las entradas del usuario.
	\item No confiar en la validación del lado del cliente; la verificación debe realizarse siempre en el servidor.
	\item Aplicar el principio de privilegio mínimo en el acceso a la base de datos, utilizando usuarios con permisos limitados.
	\item Utilizar procedimientos almacenados siempre que sea posible para encapsular las consultas SQL.
\end{itemize}
